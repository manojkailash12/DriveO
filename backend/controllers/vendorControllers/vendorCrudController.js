        ...
        try {
          if (uploadedImages.length > 0) {
            const addVehicle = new vehicle({
              ...
              car_type,
              created_at: Date.now(),
              state: state || district, // fallback to district if state not provided
              district,
              city: city || location, // fallback to location if city not provided  
              location,
              isAdminAdded: false,
              addedBy: addedBy,
...
export const showVendorVehicles = async (req, res, next) => {
  try {
    ...
    // Return empty array instead of error if no vehicles found
